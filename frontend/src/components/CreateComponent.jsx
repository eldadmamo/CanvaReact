...
const CreateComponenet = ({current_component,info,removeComponent}) => {
    // console.log(info)
    ...
    if(info.name === 'shape' && info.type === 'circle'){
      ...
      hover:border-indigo-500'
            ...
            <Element id={randValue} info={info} exId = {`${randValue}c`} />
        {
            ...
            removeComponent(info.id)} className='px-3 py-2 bg-white 
    ...
    if(info.name === 'shape' && info.type === 'triangle'){
      ...
      hover:border-indigo-500'
        >
            <div id={`${randValue}t`} className='rounded-full' 
            style={{
            width: info.width + 'px',
            height: info.height + 'px',
            background: info.color,
...
export default CreateComponenet;